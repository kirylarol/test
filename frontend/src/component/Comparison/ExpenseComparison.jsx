...
export const ExpenseComparison = ({ periods }) => {
  ...
  const fetchComparisonData = async () => {
    try {
      setLoading(true);
      const response = await instance.get('/comparison/expenses', {
        params: periods
      });
      setComparisonData(response.data);
      setLoading(false);
    } catch (err) {
      setError('Failed to load expense comparison data. Please try again.');
      setLoading(false);
    }
  };
  ...
  const fetchCategoryComparisonData = async () => {
    try {
      setLoading(true);
      const response = await instance.get('/comparison/expenses-by-category', {
        params: periods
      });
  ...
  return (
    <Container>
      <h2 className="my-4">Expense Comparison</h2>
      ...
      {loading ? (
        ...
        <>
            ...
            <>
              <Row className="mb-4">
                <Col md={12}>
                  <Card>
                    <Card.Header>
                      <h5>Total Expenses Comparison</h5>
                    </Card.Header>
                    <Card.Body>
                      <Row>
                        <Col md={4}>
                          <Card className="text-center mb-3">
                            ...
                            <Card.Body>
                              <h3 className="text-danger">{formatCurrency(comparisonData.period1.amount)}</h3>
                        ...
                        <Col md={4}>
                          <Card className="text-center mb-3">
                            ...
                            <Card.Body>
                              <h3 className={comparisonData.increased ? 'text-danger' : 'text-success'}>
                                {comparisonData.increased ? '+' : ''}{formatCurrency(comparisonData.difference)}
                              </h3>
                              <p className={comparisonData.increased ? 'text-danger' : 'text-success'}>
                                {formatPercentage(comparisonData.percentageChange)}
                              </p>
                              <p className="text-muted">
                                {comparisonData.increased ? 'Expenses increased' : 'Expenses decreased'}
                              </p>
                            </Card.Body>
                          </Card>
                        </Col>
                        ...
                        <Col md={4}>
                          <Card className="text-center mb-3">
                            ...
                            <Card.Body>
                              <h3 className="text-danger">{formatCurrency(comparisonData.period2.amount)}</h3>
                              <p className="text-muted">
                        ...
                        <Bar 
                          ...
                          options={{
                            ...
                            plugins: {
                              ...
                              title: {
                                display: true,
                                text: 'Total Expenses Comparison'
                              }
                            }
                          }}
                        />
                      ...
                      <Bar 
                        ...
                        options={{
                          ...
                          plugins: {
                            ...
                            title: {
                              display: true,
                              text: 'Expenses by Category Comparison'
                            }
                          }
                        }}
                      />
                      ...
                      <tbody>
                        {Object.entries(categoryComparisonData.categoryComparisons).map(([category, data]) => (
                          <tr key={category}>
                            ...
                            <td className={data.increased ? 'text-danger' : 'text-success'}>
          ...
          {!comparisonData && !loading && (
            <Alert variant="info">
              Select periods to compare expenses.
            </Alert>
          )}
        </>
      )}
